...
def synthetic_split():
    ...
    X_train, X_test, y_train, y_test = preprocessing.load_and_split(df)
...
def test_run_baseline_pipeline_returns_models_and_metrics(tmp_path, monkeypatch, synthetic_split):
    ...
    X_train, X_test, y_train, y_test = synthetic_split
    models, results = baseline_models.run_baseline_pipeline(X_train, y_train, X_test, y_test)
